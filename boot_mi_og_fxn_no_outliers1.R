...
# 
    ...
    # library(RcppRoll)
    # library(survival)
    # dat=fread("/Users/joshslone/Desktop/PhD/Research/KS_Data/Code/dat_base_mi.csv")

    d= dat |> mutate(V=if_else(patient %in% c("74605","pe.3397"),0,V))
    
    ## Impute exclusion of patients with KS diagnosis before 2009-12-01; 1 patient from EA should be excluded
    d = d |>mutate(exclusion_flag=if_else(is.na(ks_y_final) & V==0 & ks_y==1 & program==1,rbinom(length(program),1,1/264),0))|> filter(exclusion_flag==0) |>mutate(exclusion_flag=NULL)
    ...
    d = d |>mutate(birth_d_error_imp_gr=if_else(program==1,rbinom(length(program),1,65/824),rbinom(length(program),1,1/113))) |> mutate(birth_d_error_imp=if_else(V==0, birth_d_error_imp_gr, birth_d_error)) 
    ...
    load("/Users/joshslone/Desktop/PhD/Research/KS_Data/Code/all_cd4_1.Rda") 
    
    #load("all_val_long4.Rda")
    
    load("/Users/joshslone/Desktop/PhD/Research/KS_Data/Code/all_val_long4.Rda") 
    ...
    d3=d3 |> lazy_dt() %>% group_by(patient,id) %>% mutate(new_v = if_else(is.na(month_avg_cd4_final),NA,row_number())) |> 
  fill(new_v) |> mutate(row_diff = row_number()-new_v) |> fill(month_avg_cd4_final) |> 
    mutate(month_avg_cd4_final=if_else(row_diff <= 12, month_avg_cd4_final,NA))  |> 
    mutate(row=row_number(),cd4_1=month_avg_cd4_final) |> fill(cd4_1,.direction = "up") |> fill(new_v,.direction = "up") |> 
      mutate(cd4_final=if_else(row < 7 & new_v <=7, cd4_1, month_avg_cd4_final)) |> fill(cd4_final,.direction = "down") |> ungroup() |> as.data.frame()
    
    d3=d3 |> lazy_dt() %>% group_by(patient,id) %>% mutate(new_v = if_else(is.na(month_avg_cd4),NA,row_number())) |> 
    fill(new_v) |> mutate(row_diff = row_number()-new_v) |> fill(month_avg_cd4) |> 
    mutate(month_avg_cd4=if_else(row_diff <= 12, month_avg_cd4,NA)) |> filter(year_month>="2010-01",year_month<="2019-12") |> 
    mutate(row=row_number(),cd4_1=month_avg_cd4) |> fill(cd4_1,.direction = "up") |> fill(new_v,.direction = "up") |> 
    mutate(cd4=if_else(row < 7 & new_v <=7, cd4_1, month_avg_cd4)) |> fill(cd4,.direction = "down") |> ungroup() |> as.data.frame()
    ...
    d6 = d3 |> lazy_dt() |> mutate(cd4_rt = sqrt(cd4), cd4_final_rt = sqrt(cd4_final)) |> ungroup() |> as.data.frame()
    
    rm(d3); invisible(gc())
        
    d6$age<-as.numeric(round((as.Date(paste(d6$year_month, "-01", sep=""))-as.Date(d6$birth_d))/365.25,1))
    d6$year<-as.numeric(substring(d6$year_month,1,4)) 
    
    #####################################################################################################################
    
    ### imputation for phase 1 cd4 values based on error prone variables
    
    d6 = d6 |> group_by(patient) |> mutate(row=row_number())
    
    #mod5 = lm(cd4_rt ~  male +program + ns(age_imp,df=4) +art_ind +ks_ind+ death_ind, data= d6[!is.na(d6$cd4_rt),]) 

    mod5 = lm(cd4_rt ~  male +program + ns(age_imp,df=4) +art_ind +ks_ind+ death_ind, data= d6[d6$row==1 & !is.na(d6$cd4_rt),]) 
    ...
    ch = d7 |> lazy_dt() |> count(patient,id,male,program,V,male_final) |>mutate(male_imp_gr=if_else(male==1 & program==1,rbinom(length(program),1,328/330),male)) |> mutate(male_imp=if_else(V==0, male_imp_gr, male_final)) |> ungroup() |> as.data.frame()
    ...
    mod6 = lm(cd4_final_rt ~  male_final +program+ ns(age_final,df=4) + ns(cd4_rt_init_imp,df=4) +art_ind_final + death_ind_final +ks_ind_final, data= d7[d7$V==1 & d7$row==1 & !is.na(d6$cd4_final_rt),])
    ...
    d7_phase2$cd4_rt_imp_final=if_else(d7_phase2$row==1 & is.na(d7_phase2$cd4_final_rt),cd4_final_rt_new,d7_phase2$cd4_final_rt)
    ...
    ### Overall KS PPV and NPV
    ch2= d8 |> filter(V==1)
    npv1=sum(table(ch2$ks_ind,ch2$ks_ind_final)[1,1])/sum(table(ch2$ks_ind,ch2$ks_ind_final)[1,])
    ppv1=sum(table(ch2$ks_ind,ch2$ks_ind_final)[2,2])/sum(table(ch2$ks_ind,ch2$ks_ind_final)[2,])
    d8 = d8 |> lazy_dt() |>mutate(ks_ind_imp_gr=if_else(ks_ind==1,rbinom(length(program),1,ppv1),rbinom(length(program),1,1-npv1))) |> mutate(ks_ind_imp=if_else(V==0, ks_ind_imp_gr, ks_ind_final)) |> ungroup() |> as.data.frame()
    ...
    d8 = d8 |>mutate(death_ind_imp_gr=if_else(last_month==1 & program==1 & death_ind==1,rbinom(length(program),1,75/83),if_else(last_month==1 & program==1 & death_ind==0,rbinom(length(program),1,1-(739/741)),if_else(last_month==1 & program==0 & death_ind==1,rbinom(length(program),1,17/21),if_else(last_month==1 & program==0 & death_ind==0,0,0))))) |> mutate(death_ind_imp=if_else(V==0, death_ind_imp_gr, death_ind_final)) 
    ...
    d_prev = d8 |> lazy_dt() |> filter(as.Date(enrol_d1)>=as.Date("2010-01-01") | (as.Date(enrol_d1)>=as.Date("2009-12-01") & program==1)) |>
      ...
      filter(enrol_d1_year_month==year_month | (row_number() ==1 & enrol_d1_year_month=="2009-12")) |> mutate(year_imp = as.numeric(substring(year_month,1,4)) ) |>
    ...
    d11$year<-as.numeric(substring(d11$year_month,1,4)) 
    ...
    d_inc_a2$n = as.numeric(substring(d_inc_a2$max_date,6,7))-as.numeric(substring(d_inc_a2$min_date,6,7)) + 1
    d_inc_a2$pys=d_inc_a2$n/12
    d_inc_a2$year<-as.numeric(substring(d_inc_a2$min_date,1,4)) 
