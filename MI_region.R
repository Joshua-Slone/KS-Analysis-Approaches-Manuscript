mi_fxn <- function(dat, n){
  ...
  for(j in 1:n){
    ...
    #d_inc_a2=d_inc_a1 |> fgroup_by(patient,id,program,year,art_ind_imp,male_imp,cd4_rt_imp1,ct,birth_d_imp)|> fsummarise(ks_ind_imp=max(ks_ind_imp),min_date=min(year_month),max_date=max(year_month)) |>  roworder(patient,id,min_date)  |> fungroup()
