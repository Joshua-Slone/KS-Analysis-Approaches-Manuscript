inc_fxn <- function(dat,dat1,dat2,dat3,n){
  ...
  betas_ipw = vcovs_ipw = betas_gr = vcovs_gr = betas_ehr = vcovs_ehr = vector("list", n)
  ...
  for(i in 1:n){
    ...
    betas_ipw[[i]] = coef(ipw_inc_3)
    vcovs_ipw[[i]]  = vcov(ipw_inc_3)
    betas_gr[[i]]  = coef(fit_gr_inc_2)
    vcovs_gr[[i]]  = vcov(fit_gr_inc_2)
    betas_ehr[[i]] = coef(mod_ehr_inc)
    vcovs_ehr[[i]] = vcov(mod_ehr_inc)
    
    
    
  }
  
  ddest <- list("coef_ipw" = betas_ipw,"vcovs_ipw" = vcovs_ipw, "coef_gr" = betas_gr,"vcovs_gr" = vcovs_gr,"coef_ehr" = betas_ehr,"vcovs_ehr" = vcovs_ehr)
